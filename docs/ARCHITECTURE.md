...
- `ui/connectedElements.js` owns the connected-component picker, its keyboard focus, direction toggle, and shared library favourites/recent choices. Its preview stays outside document state. `canvas/connectionPlacement.js` finds a clear, grid-aligned position without moving existing elements. The runtime owns directional handles, magnetic target resolution, and the draft viewport; live and preview connectors use the same geometry. Creation adds the element and flow in one history/save operation, with the initial component rename merged into that entry. Cancellation restores the temporary view and creates no objects. `styles/connectionAssist.css` supplies the handles and picker; direct SVG export strips their transient graphics.
...
- `styles/presentationSpace.css` gives presentation a compact header, an optional independent panel and a playback dock outside the drawable SVG. Presentation fit measures the actual SVG viewport, glyph and connector bounds, and all groups' potential token/callout bounds. A bounded search accounts for labels' minimum screen size. ResizeObserver and layout controls refit only while session-only automatic fitting is enabled; manual pan/zoom disables it until Fit is chosen. Playback never refits between messages. Panel visibility, width and tab persist as presentation-specific settings; presentation zoom and pan do not replace the saved editor viewport. Full-screen ownership tracks whether exiting presentation should also exit browser full screen.
