...
- Drag from a component's connection handle to a destination. Alternatively, click the source handle and then the target. **Escape** cancels a connection. Keyboard users can focus a component and press **Enter** to reach its handles, then use **Tab** and **Enter** to connect.
...
Select a flow step and choose **Present** to begin there, or use **Play from here** in its expanded row. The presentation header shows the current message number, name, route and phase. Processing actions, notes and images appear alongside the diagram; messages that run together share a position in the flow overview.
